...
export interface PullbackStats {
  ...
  basisHigh: number;
  basisLow: number;
  retracePrice: number; // low (Bull) or high (Bear) of trade day
  depth: number; // e.g. 0.73 = 73% retrace
  fibBucket: string; // "0.5-0.618", "0.618-0.786", etc.
}

export interface PullbackHistoryStats {
  /** Number of days considered in the rolling window (e.g. 60) */
  window: number;

  /** Number of days that actually contributed (trend != Neutral + valid range) */
  sampleSize: number;

  /** Average pullback depth (0–1, where 1 = full prior-day range) */
  meanDepth: number | null;

  /** Median pullback depth (0–1) */
  medianDepth: number | null;

  /** Minimum and maximum observed depth within the window */
  minDepth: number | null;
  maxDepth: number | null;

  /**
   * Frequency distribution by Fib-style bucket:
   * "0-0.382", "0.382-0.5", "0.5-0.618", "0.618-0.786", "0.786-1.0", "1.0+"
   */
  bucketCounts: Record<string, number>;
}

// Single trade candidate generated by the engine
export type PlacementKind = "MARKET" | "PENDING_LIMIT";
...
export interface SymbolScanResult {
  ...
  /** Aggregated historical pullback profile over a rolling window */
  pullbackHistory?: PullbackHistoryStats;

  /** Simplified pullback summary for sweet-spot evaluation */
  typicalPullback?: {
    lookbackDays: number;
    samples: number;
    meanPct: number | null;
